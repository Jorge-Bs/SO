...
void Processor_DecodeAndExecuteInstruction() {
	...
	switch (operationCode) {
		...
		case DIV_INST: 
			if (operand2 == 0)
				Processor_RaiseException(DIVISIONBYZERO); //V4-Ej1-b
				//Processor_RaiseInterrupt(EXCEPTION_BIT); antes  
			else {
				registerAccumulator_CPU=operand1 / operand2;
				registerPC_CPU++;
			}
		...
		case HALT_INST: 
			...
			if(Processor_PrivilegedMode()==1){
				...
				Processor_RaiseException(INVALIDPROCESSORMODE);//V4-Ej1-b
				//Processor_RaiseInterrupt(EXCEPTION_BIT);antes
			}
			//Fin V1-EJ15
			break;
		...
		default : 
			...
			Processor_RaiseException(INVALIDINSTRUCTION);//V4-Ej3-a
