	...
	switch (registerCTRL_MMU) {
  	case CTRLREAD:
			if (Processor_PSW_BitState(EXECUTION_MODE_BIT)){ // Protected mode
				if (registerMAR_MMU < MAINMEMORYSIZE && registerMAR_MMU >= 0){//V4-ej1-c el registerBase_MMU>0
					...
					Processor_RaiseException(INVALIDADDRESS);//V4-Ej1-c
				}
			}
			else // Non-Protected mode
				if (registerMAR_MMU<registerLimit_MMU  && registerMAR_MMU >=0) { //V4-ej1-c el registerBase_MMU>0
