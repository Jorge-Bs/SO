...
int OperatingSystem_CreateProcess(int indexOfExecutableProgram) {
	...
	processTable[PID].internalFrag= 100-(((float)processSize/partitionsTable[partitionIndex].size)*100);
...
void OperatingSystem_HandleClockInterrupt(){ 
	...
	OperatingSystem_CambiarPrioridades();//Simulacro
...
//Simulacro2
void OperatingSystem_CambiarPrioridades(){
	int pid, tamanoProceso, tamanoParticion, particion,prioridad, prioridadAntigua;
	float fragmentacion;


	for(int i=0;i<numberOfSleepingProcesses;i++){
		pid=sleepingProcessesQueue[i].info;
		tamanoProceso=processTable[pid].processSize;
		particion = processTable[pid].partition;

		tamanoParticion=partitionsTable[particion].size;

		fragmentacion = processTable[pid].internalFrag;//((float) tamanoProceso/tamanoParticion)*100;

		if(fragmentacion>25.0 ){
			prioridadAntigua=processTable[pid].priority;
			prioridad=processTable[pid].priority-5;
			if(prioridad<0){
				prioridad=0;
			}
			processTable[pid].priority=prioridad;

			ComputerSystem_DebugMessage(TIMED_MESSAGE,146,EXAM,pid,programList[processTable[pid].programListIndex]->executableName,prioridadAntigua,prioridad,particion,fragmentacion);
		}
	}
}
